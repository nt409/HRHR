...
concs1D = [exp(- PARAMS.delta_1 * (xx - 1456))
           if xx > 1456 else 0 for xx in time]
concs2D = [exp(- PARAMS.delta_1 * (xx - 1700))
...
d = DoseResponse(dose_resp_data, effect_vs_time_data, filename)

d.fig.write_image('../outputs/figures/paper_figs/hires/fig1.jpg', scale=4)
