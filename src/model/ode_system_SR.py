...
class ODESystemWithinSeasonSex(BaseSystem):
    ...
    def system(self, t, y):
        ...
        dydt = [self.growth_fn(A, t)
            ...
            - (self.senescence_fn(t)) * ERS - pars.gamma *
            (self.fcide2.effect(conc_2)) * ERS,

            S*(pars.beta/A) * ((1-ws_prop)*(ISR + PSR)
                               + ws_prop*(I_div*(ISS + ISR)*(ISR + IRR)
                                          + P_div*(PSS + PSR)*(PSR + PRR))
                               ) * (self.fcide1.effect(conc_1))
            - (self.senescence_fn(t)) * ESR - pars.gamma *
            (self.fcide1.effect(conc_1)) * ESR,
            ...
            - (self.senescence_fn(t)) * ESS - pars.gamma *
            (self.fcide1.effect(conc_1))*(self.fcide2.effect(conc_2)) * ESS,
            ...
            pars.gamma * (self.fcide1.effect(conc_1)) *
            (self.fcide2.effect(conc_2)) * ESS - pars.mu * ISS,
