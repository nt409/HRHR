...
class RandomPars:
    """Generates random parameters for parameter scan or model run
    ...
    >>>RP.get_inoc_dict(pars["RS"], 
    ... pars["SR"], pars["RR"])
    >>>RP.fung_parms = RP.get_fung_parms_dict(
    ... pars["omega_1"], 
    ... pars["omega_2"],
    ... pars["delta_1"], 
    ... pars["delta_2"],
    ...
    def find_pars(self):
        ...
        while pars_are_valid is False:
            ...
            rfs1_power = np.random.uniform(
                low=conf["RF_single"][0], high=conf["RF_single"][1])
            rfs2_power = np.random.uniform(
                low=conf["RF_single"][0], high=conf["RF_single"][1])
            rfD_power = np.random.uniform(
                low=conf["RF_double"][0], high=conf["RF_double"][1])

            rfs1 = 10**(rfs1_power)
            rfs2 = 10**(rfs2_power)
            rfD = 10**(rfD_power)

            om_1 = np.random.uniform(
                low=conf["omega"][0], high=conf["omega"][1])
            om_2 = np.random.uniform(
                low=conf["omega"][0], high=conf["omega"][1])

            delt_1 = np.random.uniform(
                low=conf["decay_rate"][0], high=conf["decay_rate"][1])
            delt_2 = np.random.uniform(
                low=conf["decay_rate"][0], high=conf["decay_rate"][1])

            theta_1 = np.random.uniform(
                low=conf["theta"][0], high=conf["theta"][1])
            theta_2 = np.random.uniform(
                low=conf["theta"][0], high=conf["theta"][1])

            sr_prop = np.random.uniform(low=conf["SR"][0], high=conf["SR"][1])

            fungicide_params = self.get_fung_parms_dict(
                om_1, om_2, delt_1, delt_2, theta_1, theta_2)
            ...
            pars_are_valid = self._check_validity(
                fungicide_params, pathogen_pars)
    ...
    def _get_yield_these_doses(self, fungicide_params, pathogen_pars, d1, d2):
        ...
        conf_single = SingleConfig(
            1, None, None, d1, d1, d2, d2, primary_inoculum=self.inoc)
