...
class PostProcess:
    ...
    def __init__(self, par_str, folder=None):

        self.folder = "./param_scan/outputs" if folder is None else folder

        df_in = pd.read_csv(
            f"{self.folder}/combined/output_summary_df_{par_str}.csv")

        self.df = df_in.drop(["Unnamed: 0"], axis=1)

        self.par_df = pd.read_csv(
            f"{self.folder}/combined/output_par_df_{par_str}.csv")
        ...
        for ii in range(df_test.shape[0]):
            ...
            if plot:
                conf_str = conf_str.replace(
                    "param_scan/",
                    f"param_scan/run={this_run_ind}_"
                )
    ...
    def re_run_IVT(self, NDoses, run_indices):
        ...
        for ii in range(df_test.shape[0]):
            ...
            conf_str = rp.grid_conf.config_string_img
            conf_str = conf_str.replace(
                "param_scan/", f"param_scan/run={this_run_ind}_")
...
class ProcessedDF:
    ...
    def _process_df(self, par_df, data_in):
        ...
        for string in strats:
            data.loc[:, string + "%"] = 100 * \
                data[string + "EL"]/data["max_grid_EL"]
        ...
        data['ESFYL_diff_from_opt'] = data["c_E_lowDoseMaxEL"] - \
            data["max_grid_EL"]
    ...
    def _check_if_never_failed(self, df):
        ...
        if df[df["O_fullDoseEL"] <= 0].shape[0]:
            ...
            print(
                f"{n_never_fail} runs never failed - need longer n_years. Filtering them out for now.")
