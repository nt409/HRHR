...
def main(config):
    ...
    print(dfp
          .loc[dfp["c_R_maxContEL"] - 4 >= dfp["c_E_maxContEL"],
               ["c_R_maxContEL", "c_E_maxContEL"]]
          .mean()
          )
    ...
    print(dfp
          .loc[dfp["c_R_maxContEL"] < dfp["c_E_maxContEL"],
    ...
    print(100*sum(
          dfp
          .loc[dfp["ESFY_diff_from_opt"] <= -2, ["ESFY_diff_from_opt"]]
          .count()
          )/500
          )
    print(100*sum(
          dfp
          .loc[dfp["ESFYL_diff_from_opt"] <= -2, ["ESFYL_diff_from_opt"]]
          .count()
          )/500
          )
