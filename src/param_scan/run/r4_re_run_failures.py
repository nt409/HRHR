...
def print_it():
    ...
    print("FAILURES:")
    ERFB_sub_opt = df.loc[df["c_R_maxCont%"]<100, ["max_grid_EL", "c_R_maxContEL", "I_R_best_value", "c_E_maxContEL"]]
    print(ERFB_sub_opt)
    print(f"n failures: {ERFB_sub_opt.shape[0]}")

    print("ESFY 'outperforms' ERFB:")
    ESFY_outperforms = df.loc[df["c_R_maxContEL"]<df["c_E_maxContEL"], ["max_grid_EL", "c_E_maxContEL", "c_R_maxContEL", "I_R_best_value"]]
...
if __name__=="__main__":
    ...
    run_attrs = [
        ...
        #### dict(run = 295, DS_lim=[0.6,0.75]), # M?
        # dict(run = 295, DS_lim=[0.67,0.7]), # Y
