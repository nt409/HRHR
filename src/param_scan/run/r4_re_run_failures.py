...
def check_outcome_RFB(run_attrs):
    ...
    for data in run_attrs:
        ...
        DS_lim = data["DS_lim"]
        PP.re_run_cont_RFB(NDoses=NDoses, N_cont_doses=N_cont_doses, DS_lim=DS_lim, run_indices=[run])
...
def check_outcome_SFY(run_attrs):
    ...
    for data in run_attrs:
        ...
        N_cont_doses = data["N_cont_doses"]
        DS_lim = data["DS_lim"]
        PP.re_run_cont_SFY(NDoses=NDoses, N_cont_doses=N_cont_doses, DS_lim=DS_lim, run_indices=[run])
...
if __name__=="__main__":
    ...
    run_attrs = [
        ...
        dict(run = 91,  DS_lim=[0.4,0.5], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # N
        dict(run = 183, DS_lim=[0.3,0.45], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # N
        dict(run = 227, DS_lim=[0.6,0.7], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # M?
        dict(run = 241, DS_lim=[0.3,0.6], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # N
        dict(run = 295, DS_lim=[0.6,0.75], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # M?
        dict(run = 478, DS_lim=[0.2,0.6], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # N

        # ESFY outperformed ERFB
        dict(run = 265, DS_lim=[0.44,0.46], NDoses=N_dose_def, N_cont_doses=N_cont_ds), # N
        ]
